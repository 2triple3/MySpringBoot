...
import com.springboot.system.security.JwtAuthenticatioToken;
import com.springboot.system.util.PasswordUtils;
import com.springboot.system.util.SecurityUtils;
...
//		user.setUsername((String)userInfo.get("username"));
	...
	public Map<String, Object> doLogin(@RequestParam("username") String username, @RequestParam("password") String password, HttpServletRequest request) {
		System.out.println("username_doLogin:" + username + ",password_doLogin:" + password);
		List<SysUser> userlist = sysUserServiceImpl.findUserInfoByUsername(username);
		String status = "0";
		Iterator it = userlist.iterator();
//		if (userlist.size() > 0) {
...
//					|| password.equals(((SysUser) userlist.get(0)).getPassword())) {
//				System.out.println(((SysUser) userlist.get(0)).toString() + "::::" + ((SysUser) userlist.get(0)).getPassword());
//				status = "1";
//			}
//		}
		if (userlist.size() > 0) {
			if (PasswordUtils.matches(((SysUser) userlist.get(0)).getSalt(), password, ((SysUser) userlist.get(0)).getPassword())) {
				System.out.println(((SysUser) userlist.get(0)).toString() + "::::" + ((SysUser) userlist.get(0)).getPassword());
				status = "1";
			}
		}
